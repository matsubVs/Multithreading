...
class Protocol:
    ...
    def send(self, op_code, param):
        def process_sending():
            print(f'Operation is in action with {param=}')
    ...
    def process(self, op_code, param):
        print(f'Processing operation {op_code=} with {param=}')
        time.sleep(3)

        finished = randint(0, 1) == 1
...
class BankTerminal:
    ...
    def purchase(self, amount):
        ...
        t = threading.Thread(target=purchase_process)
        t.start()
